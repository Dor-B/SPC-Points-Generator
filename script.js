...
function mToPx(point){
    return {x: (point.x + (parameters.startX + (parameters.isBackwards ? parameters.robotW - parameters.robotCenterToBack : parameters.robotCenterToBack))) * (parameters.imageW / parameters.fieldW),
...
function pxToM(point){
	return {x: point.x * (parameters.fieldW / parameters.imageW) - (parameters.startX + (parameters.isBackwards ? parameters.robotW - parameters.robotCenterToBack : parameters.robotCenterToBack)),
...
//    parameters.pathPoints.map(pt => {x:pt.x + Math.random() / 1000, y:pt.y});
    parameters.pathPoints.map(function(pt){return {x:pt.x + Math.random() / 1000, y:pt.y}});
	var counter = 0;
